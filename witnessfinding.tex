...
\begin{itemize}
\item Adaptive $\mc{Q}_0$ upper bound is $O(n)$ (\autoref{thm:naive}).
\item Adaptive $\mc{Q}_0$ lower bound is $\Omega(n)$ (lower bound for all queries).
\item Nonadaptive $\mc{Q}_0$ upper bound is $O(n^2)$ (\autoref{thm:vv}).
\item Nonadaptive $\mc{Q}_0$ lower bound is $\Omega(n^2)$ (\autoref{cor:q0lower}).
\item Adaptive $\mc{Q}_1$ upper bound is $O(n^2)$ (from nonadaptive upper bound).
\item Adaptive $\mc{Q}_1$ lower bound is $\Omega(n^2)$ (but only algorithms with high success probability \autoref{cor:isolationalg}).
\item Nonadaptive $\mc{Q}_1$ upper bound is $O(n^2)$ (\autoref{cor:q1upper}).
\item Nonadaptive $\mc{Q}_1$ lower bound is $\Omega(n^2)$ (\autoref{cor:q1lower}).

\end{itemize}

\todo{Can we improve the upper and lower bounds for adaptive $\mc{Q}_1$ witness-finding algorithms?}
