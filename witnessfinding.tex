...
In other words, $A^{=0}_W$ acts as an oracle for \textsc{Satisfiability} and $A^{=1}_W$ for \textsc{Unique Satisfiability}.
(For context, $\coNP \subseteq \US \subseteq \BH_2 \subseteq \P^\NP$.)
The equivalence of the $\NP$ and $\coNP$ oracles is given by the following proposition.

\begin{proposition}
  Suppose $W$ is a witness set.
  There is a witness-finding algorithm with oracle $A^{=0}_W$ if and only if there is a witness-finding algorithm with oracle $A^{>0}_W$.
...
\section{Reductions to counting classes}
...
There is a superficial similarity to the question ``is $\P^\PP$ a subset of $\P^{\CeP}$?'' and the question ``does a witness-finding algorithm with oracle $A^{\geq k}_W$ imply a witness-finding algorithm with oracle $A^{= k}_W$?''
However, we run into a few problems when trying to prove that one implies the other.

\begin{conjecture}
  If $\P^\PP \subseteq \P^{\CeP}$, then a witness-finding algorithm with oracle $A^{\geq k}_W$ implies a witness-finding algorithm with oracle $A^{= k}_W$.
\end{conjecture}
\begin{proof}[Broken proof]
  ...
  \end{itemize}
  \todo{Problem 1: okay, this is a randomized algorithm, not a deterministic one.}
  By our hypothesis that $\P^\PP \subseteq \P^{\CeP}$, there is also an oracle machine $M'$ that decides $L$ with access to a $\CeP$ oracle.
